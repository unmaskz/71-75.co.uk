...
export default ({ children, className, block, reverse, toolbar, column }: Props): JSX.Element => {
    ...
    const reverseClass = reverse !== undefined ? styles.buttonGroup___reverse : '';
    ...
    return (
        <div
            className={`${styles.buttonGroup} ${className ? className : ''} ${blockClass} ${columnClass} ${reverseClass} ${toolbarClass}`}
        >
            {children}
        </div>
    );
